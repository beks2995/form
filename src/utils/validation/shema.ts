...
const valueFunc = (
  ...
  maxVal: number,
  maxText: string
) => func.min(minVal, minText).max(maxVal, maxText)
...
export const schema = object().shape({
  ...
  AreaAll: valueFunc(
    typeErrFunc(requiredFunc(number())),
    1,
    'Площадь не должна быть меньше 1',
    1500,
  ...
  Floor: valueFunc(
    typeErrFunc(requiredFunc(number())),
    -4,
    'Этаж не может быть меньше минус 4',
    48,
  ...
  Ceil: valueFunc(
    typeErrFunc(requiredFunc(number())),
    1,
    'Недопустимая высота потолков',
    15,
    'Максимальная высота потолка 15м'
  ),
})
